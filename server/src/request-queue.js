...
export default class RequestQueue {
  ...
  constructor(requestQueues, pendingResponses) {
    ...
    // Store ArrayBuffer bodies separately to avoid JSON serialization issues
    this.requestBodies = new Map(); // request_id -> ArrayBuffer
  }

  /**
   * Queues a public request and waits for response from tunnel client
   * @param {string} tunnelId - Tunnel ID to queue request for
   * @param {object} requestData - Serialized request data (with ArrayBuffer body)
  ...
  async queueRequest(tunnelId, requestData, timeout = this.requestTimeout) {
    ...
    // Store ArrayBuffer body separately
    if (requestData.body instanceof ArrayBuffer) {
      this.requestBodies.set(requestData.id, requestData.body);
    }

    // Create request data for queue (without ArrayBuffer body)
    const queuedRequest = {
      ...requestData,
      body: requestData.body instanceof ArrayBuffer ? requestData.body.byteLength : 0, // Store size only
    ...
    return new Promise((resolve, reject) => {
      ...
      setTimeout(() => {
        ...
        if (pending) {
          ...
          this.requestBodies.delete(requestData.id);
  ...
  async waitForRequest(tunnelId, timeout = this.pollTimeout) {
    ...
    if (queue && queue.length > 0) {
      ...
      // Restore ArrayBuffer body for transmission
      const storedBody = this.requestBodies.get(request.id);
      if (storedBody) {
        request.body = storedBody;
      } else {
        request.body = new ArrayBuffer(0); // Fallback to empty ArrayBuffer
      }
   ...
   * @param {object} responseData - Response data from client (with ArrayBuffer body)
   */
  async resolveRequest(requestId, responseData) {
    ...
    // Clean up stored request body
    this.requestBodies.delete(requestId);

    // Resolve the promise with ArrayBuffer response
  ...
  cleanupExpiredRequests() {
    ...
    if (cleanedRequests > 0 || cleanedQueues > 0 || cleanedBodies > 0) {
      console.log(`Request queue cleanup: removed ${cleanedRequests} expired requests, ${cleanedQueues} empty queues, ${cleanedBodies} orphaned bodies`);
    }
  }
  ...
  cancelTunnelRequests(tunnelId) {
    ...
    const queue = this.requestQueues.get(tunnelId);
    if (queue) {
      // Reject all queued requests and clean up bodies
      for (const request of queue) {
        this.rejectExpiredRequest(request.id);
      }
      this.requestQueues.delete(tunnelId);
    ...
    for (const requestId of toCancel) {
      ...
      if (pending) {
        this.pendingResponses.delete(requestId);
        this.requestBodies.delete(requestId); // Clean up body
